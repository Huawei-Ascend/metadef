...
BufferFusionPattern &BufferFusionPattern::AddOpDesc(const std::string &desc_name, const std::vector<std::string> &types,
  ...
  if (repeate_min > repeate_max) {
    GELOGW(
        "Repeat_min can not lager than repeate_max, desc name is [%s], min is "
        "[%ld], max is [%ld]",
...
void BufferFusionPattern::UpdateSkipStatus(BufferFusionOpDesc *op_desc) {
  if (op_desc->out_branch_type == TBE_OUTPUT_BRANCH_MULTI) {
    for (auto &inputdesc : op_desc->inputs) {
      if (inputdesc->types.size() != op_desc->types.size()) {
        continue;
      }
      bool is_same_type = true;
      for (uint32_t i = 0; i < inputdesc->types.size(); i++) {
        if (inputdesc->types[i] != op_desc->types[i]) {
          is_same_type = false;
          break;
        }
      }
      if (is_same_type && inputdesc->ignore_output_num == true) {
        for (uint32_t i = inputdesc->repeate_min; i < inputdesc->repeate_max; i++) {
          inputdesc->multi_output_skip_status[i] = SkipStatus::AVAILABLE;
        }
      }
    }
  }
