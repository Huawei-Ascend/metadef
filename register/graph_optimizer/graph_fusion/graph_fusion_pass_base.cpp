...
Status GraphFusionPassBase::RunOnePattern(ge::ComputeGraph &graph, const FusionPattern &pattern, bool &changed) {
  ...
  GELOGD("GraphId[%d], GraphFusionPass[%s]: pattern=%s, matched_times=%d, effected_times=%d.", graph_id, GetName().c_str(),
         pattern.GetName().c_str(), static_cast<int32_t>(mappings.size()), effect_times);
  return SUCCESS;
}
