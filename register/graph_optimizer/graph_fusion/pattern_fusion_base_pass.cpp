...
Status PatternFusionBasePass::SetDataDumpAttr(vector<ge::NodePtr> &original_nodes, vector<ge::NodePtr> &fus_nodes) {
  std::map<ge::InDataAnchorPtr, ge::OutDataAnchorPtr>::iterator anchor_iter;

  for (anchor_iter = origin_op_anchors_map_.begin(); anchor_iter != origin_op_anchors_map_.end(); anchor_iter++) {
...
void PatternFusionBasePass::RecordOutputAnchorMap(ge::NodePtr output_node) {
  for (auto output_anchor : output_node->GetAllOutDataAnchors()) {
    if (output_anchor == nullptr) {
      continue;
    }

    for (auto peer_in_anchor : output_anchor->GetPeerInDataAnchors()) {
      if (peer_in_anchor == nullptr) {
        continue;
      }

      // Record anchor map
      origin_op_anchors_map_[peer_in_anchor] = output_anchor;
    }
  }
}
}  // namespace fe
