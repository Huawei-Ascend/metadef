...
using ParseSubgraphFuncV1 = std::function<Status(const ge::AscendString &subgraph_name, const ge::Graph &graph)>;
...
class FMK_FUNC_HOST_VISIBILITY FMK_FUNC_DEV_VISIBILITY OpRegistrationData {
 public:
  ...
  ATTRIBUTED_DEPRECATED(OpRegistrationData &ParseSubgraphPostFn(const ParseSubgraphFuncV1 &))
  OpRegistrationData &ParseSubgraphPostFn(const ParseSubgraphFunc &subgraph_post_fn);

  OpRegistrationData &ParseSubgraphPostFn(const ParseSubgraphFuncV1 &subgraph_post_fn);
  ...
  Status GetParseSubgraphPostFn(ParseSubgraphFuncV1 &func) const;
